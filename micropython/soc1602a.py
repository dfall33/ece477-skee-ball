...
class SOC1602A:
    ...
    def __init__(self):
        ...
        self.sck = Pin(self.SCK_PIN_NUM, Pin.OUT)

        self.mosi = Pin(self.MOSI_PIN_NUM, Pin.OUT)

        self.miso = Pin(self.MISO_PIN_NUM, Pin.OUT)

        self.cs = Pin(self.CS_PIN_NUM, Pin.OUT)
    ...
    def spi_cmd(self, cmd: int) -> None:
        self.chip_select()
        self.spi.write(self.prepare_frame(cmd))
        self.chip_deselect()

    def spi_data(self, data: int) -> None:
        self.chip_select()
        data |= 0x200
        self.spi.write(self.prepare_frame(data))
        self.chip_deselect()
    ...
    def write_progress(self, progress: float, line: int = 0) -> None:
        ...
        if line == 0:
            ...
            self.cpi_cmd(self.SECOND_LINE)
        else:
            return

        max_progress_chars = 10
        num_progress_chars = int(progress * max_progress_chars)
        for i in range(max_progress_chars):

            if i < num_progress_chars:
                self.spi_data(self.PROGRESS_CHAR)
            else:
                self.spi_data(self.WHITESPACE)
